...
import com.library.im.cache.DbCache;
...
import com.library.im.domain.InviteMessage.InviteMesageStatus;

public class DemoDBManager {
    static private DemoDBManager dbMgr;
    private DbOpenHelper dbHelper;
    
    private DemoDBManager(Context context){
        dbHelper = DbOpenHelper.getInstance(context);
    }
    
    public static synchronized DemoDBManager getInstance(Context context){
        if(dbMgr == null){
            dbMgr = new DemoDBManager(context);
        }
        return dbMgr;
    }
    ...
    synchronized public void loadCache(LruCache<String, EaseUser> users){
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + UserDao.TABLE_NAME /* + " desc" */, null);
            while (cursor.moveToNext()) {
    ...
    synchronized public EaseUser get(String username){
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        EaseUser user = null;
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + UserDao.TABLE_NAME /* + " desc" */ +
                    " where " + UserDao.COLUMN_NAME_ID + "='"+username+"'", null);
            while (cursor.moveToNext()) {
                String nick = cursor.getString(cursor.getColumnIndex(UserDao.COLUMN_NAME_NICK));
                String avatar = cursor.getString(cursor.getColumnIndex(UserDao.COLUMN_NAME_AVATAR));
